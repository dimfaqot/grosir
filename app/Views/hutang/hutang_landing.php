...
<?php foreach ($data as $k => $i): ?>
    <div class="card text-bg-dark mb-3" data-menu="<?= $i['customer']; ?>">
        ...
        <div class="card-body d-flex justify-content-between ps-4">
            ...
            <div>
                <button class="btn btn-sm btn-secondary btn_detail" data-total_biaya="<?= $i['total_biaya']; ?>" data-customer="<?= $i['customer']; ?>" data-customer_id="<?= $i['customer_id']; ?>"><i class="fa-solid fa-circle-info"></i></button>
                <button class="btn btn-sm btn-success mx-1 btn_wa" data-total_biaya="<?= $i['total_biaya']; ?>" data-customer_id="<?= $i['customer_id']; ?>"><i class="fa-brands fa-whatsapp"></i></button>
                <button class="btn btn-sm btn-light btn_kasir" data-customer_id="<?= $i['customer_id']; ?>" data-customer="<?= $i['customer']; ?>"><i class="fa-solid fa-arrow-up-right-from-square"></i> Bayar</button>
            </div>
        </div>
    </div>
<?php endforeach; ?>
...
<script>
    ...
    $(document).on("click", ".btn_detail", function(e) {
        ...
        post("hutang/detail", {
            ...
            let html = build_html(customer, "offcanvas");
    ...
    $(document).on('click', '.btn_wa', function(e) {
        ...
        post("hutang/wa", {
            ...
            if (res.status == "200") {
                ...
                text += 'Yth. *' + res.data2.pj + ' [' + res.data2.customer + ']*%0a%0a';
    ...
    const kasir = (super_total, customer_id) => {
        let html = ``;
        html += `<div class="border border-secondary rounded p-3">
                    ...
                    <div class="d-grid gap-2 mt-4">
                        <button class="btn btn-info btn_bayar" data-customer_id="${customer_id}" style="width:100%"><i class="fa-solid fa-arrow-right-to-bracket"></i> BAYAR</button>
                    </div>
                </div>`;
    ...
    $(document).on('click', '.btn_kasir', function(e) {
        ...
        post("hutang/kasir", {
            ...
            let html = build_html(customer.toUpperCase(), "offcanvas");
            html += kasir(res.data, customer_id);
