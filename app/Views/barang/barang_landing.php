...
<?php foreach ($data as $k => $i): ?>
    <div class="card text-bg-dark mb-3" data-menu="<?= $i['barang']; ?>">
        ...
        <div class="card-body d-flex justify-content-between ps-4">
            <div class="text-secondary"><small><?= angka($i['harga']) . " stok: [" . angka($i['qty']) . "] isi: [" . angka($i['isi']) . "]"; ?></small></div>
            <div>
...
<script>
    let form_input = (order, id) => {
        let tipe = <?= json_encode(options("tipe")); ?>;
        let jenis = <?= json_encode(options("Kantin")); ?>;
        let satuan = <?= json_encode(options("Satuan")); ?>;
        ...
        html += `<div class="form-floating mb-3">
                        <select class="form-select bg-dark text-light border-secondary rounded" name="satuan">`;
        if (order == "Add") {
            html += `<option selected value="" required>Pilih Satuan</option>`;
        }
        satuan.forEach(e => {
            html += `<option ${(e.value==data.satuan?"selected":"")} value="${e.value}">${e.value}</option>`;
        ...
        html += `<div class="form-floating mb-3">
                        <select class="form-select bg-dark text-light border-secondary rounded" name="tipe">`;
        if (order == "Add") {
            html += `<option selected value="" required>Pilih Tipe</option>`;
        }
        tipe.forEach(e => {
            html += `<option ${(e.value==data.tipe?"selected":"")} value="${e.value}">${e.value}</option>`;

        })
        html += `</select>
                        <label class="text-secondary">Tipe</label>
                    </div>`;
        ...
        html += `<div class="form-floating mb-3">
                        <input type="text" name="isi" ${(order=="Edit"?'value="'+angka(data.isi)+'"':"")} class="form-control bg-dark text-light angka" placeholder="Isi" required>
                        <label class="text-secondary">Isi</label>
                    </div>`;
        if (order == "Edit") {
