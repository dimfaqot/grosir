...
<?php foreach ($data as $k => $i): ?>
    <div class="card text-bg-dark mb-3" data-menu="<?= $i['toko']; ?>">
        <div class="card-header"><?= ($k + 1) . ". " . $i['toko']; ?></div>
        <div class="card-body d-flex justify-content-between ps-4">
            <div class="text-secondary"><small><?= $i['sales'] . " [" . $i['alamat'] . "]"; ?></small></div>
            <div>
                <button class="btn btn-sm btn-success mx-1 btn_wa" data-toko_id="<?= $i['id']; ?>"><i class="fa-brands fa-whatsapp"></i></button>
...
<script>
    let form_input = (order, id) => {
        ...
        let html = `<div class="form-floating mb-3">
                        <input type="text" name="toko" ${(order=="Edit"?'value="'+data.toko+'"':"")} class="form-control bg-dark text-light" placeholder="Toko" required>
                    ...
                    <div class="form-floating mb-3">
                        <input type="text" name="sales" ${(order=="Edit"?'value="'+data.sales+'"':"")} class="form-control bg-dark text-light" placeholder="Sales" required>
                        <label class="text-secondary">Sales</label>
                    </div>
                     <div class="form-floating mb-3">
